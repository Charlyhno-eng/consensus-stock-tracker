...
import { Box, Button, Container, Grid, CardContent, Typography, CircularProgress } from '@mui/material';
import { interpretNoteMediane } from '@/utils/reusableFunctions';
import CustomCard from '@/components/CustomCard/CustomCard';

type SectorConsensusProps = {
  title: string;
  apiPrefix: "europe" | "amerique";
}
...
export default function SectorConsensus({ title, apiPrefix }: SectorConsensusProps) {
  ...
  const handleFetchSector = async (sector: string) => {
    ...
    try {
      const res = await fetch(`/api/consensus/${apiPrefix}?sector=${sector}`);
        ...
        <>
          ...
          <Grid container spacing={3}>
            {Object.entries(data).map(([stockName, stockData]) => (
              <Grid key={stockName} size={{ xs: 12, md: 6 }}>
                <CustomCard>
                  <CardContent>
